...
function App() {
  ...
  const handleMapClick = useCallback((latlng) => {
    if (activeTab === 'shelters') {
      setDraftCoords({ lat: latlng.lat, lng: latlng.lng });
    }
  }, [activeTab]);
  ...
  const handleUpdateStatus = useCallback((id, newStatus) => {
    ...
    if (selectedSignal?.id === id) {
      setSelectedSignal(prev => ({ ...prev, status: newStatus }));
    }
  }, [selectedSignal]);
  ...
  const handleAddShelter = (data) => {
    const newShelter = { ...data, id: `sh-${Date.now()}` };
    setShelters(prev => [newShelter, ...prev]);
  };

  const handleUpdateShelter = (id, data) => {
  ...
  const handleAddMedical = (data) => {
    const newMed = { ...data, id: `med-${Date.now()}` };
    setMedicalCenters(prev => [newMed, ...prev]);
  };

  const handleUpdateMedical = (id, data) => {
    setMedicalCenters(prev => prev.map(s => s.id === id ? { ...s, ...data } : s));
  };

  const handleDeleteMedical = (id) => {
    setMedicalCenters(prev => prev.filter(s => s.id !== id));
  };
  ...
  return (
    <div className="app-container">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', overflow: 'hidden' }}>
        {/* Layer Controls Float above or inside Map */}
