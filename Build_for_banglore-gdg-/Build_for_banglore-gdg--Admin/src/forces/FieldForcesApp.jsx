...
const FieldForcesApp = ({ onBack }) => {
  ...
  const handleFileChange = (e) => {
    ...
    files.forEach(file => {
      ...
      reader.onloadend = () => {
        setMediaFiles(prev => [...prev, { file, preview: reader.result }]);
  ...
  const handleReport = async () => {
    if (isReporting) {
      ...
      for (let media of mediaFiles) {
        try {
          const res = await fetch('http://localhost:5000/api/upload', {
            ...
            body: JSON.stringify({ image: media.preview })
          });
          if (res.ok) {
      ...
      const newReport = {
        ...
        isForceReport: true,
        mediaCount: uploadedUrls.length > 0 ? uploadedUrls.length : mediaCount,
        mediaUrls: uploadedUrls
      };

      // Send the report to the bridge server
      try {
        ...
        console.error("SOS API failed, falling back to localStorage", e);
      }

      // Save to localStorage so AdminApp can see it if bridge is down
      ...
      <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
        ...
        {!isReporting ? (
          ...
          <div style={{
              ...
              <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px' }}>
                {mediaFiles.map((media, idx) => (
                  <div key={idx} style={{ position: 'relative', width: '60px', height: '60px', flexShrink: 0 }}>
                    <img src={media.preview} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px' }} />
                  </div>
                ))}
              </div>
            )}
            ...
            >
              <Send size={20} />
              {uploading ? 'Uploading...' : 'Submit Live Report'}
            </button>
          </div>
        )}
